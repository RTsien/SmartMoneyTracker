...
3. **市场微观结构扩展** ⚠️ *仅保留接口；没有商业 Level-2 数据源时不启用*
   - 保留未来接入合规商业数据源所需的扩展接口
   - 完整盘口分析、订单失衡率（SOIR）和算法交易足迹识别不在当前路线图内
   - 当前免费数据源 AkShare、Tushare 和 yfinance 不会生成 Level-2 信号
...
│   ├── microstructure_signals.py  # 未启用的 Level-2 扩展接口
...
- **Level-2 数据** ⚠️ **未包含，仅保留扩展接口**
  - 潜在数据源：东方财富 Choice、万得等商业数据提供商
  - 在确定合规商业数据源前，完整微观结构分析不进入当前范围
  - 架构仅保留兼容数据源的扩展入口
- **机构持仓**:
...
- **资金流向**:
  ...
  - 南向资金: Eastmoney API
- **潜在公告与新闻来源**：巨潮资讯网、交易所官网；更广泛的接入保留在远期 backlog
...
- 港股披露易公告数据接入
- 新闻和基本面催化剂分析
- 通用持久化数据库：仅在需要保存历史评分或更丰富的定时任务状态时引入；现有 TTL 缓存和公告存储继续保持专项用途

### 明确不进入当前路线图

- **机器学习集成**：在具备可靠标签和时点安全回测前暂不推进，避免制造伪精度
- **策略优化引擎**：当前阶段不引入自动参数搜索，避免过拟合
- **社区共享平台**：偏离项目作为专注分析工具的核心定位，予以废弃
- **完整 Level-2 微观结构分析**：在确定合规商业数据源前不推进，仅保留扩展接口
- **逐笔实时监控**：由符合当前日线数据能力的收盘后定时扫描替代
